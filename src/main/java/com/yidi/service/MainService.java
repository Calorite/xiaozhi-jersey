	...
	public ReturnInfo answerTypesQuestion(List<ReturnInfo> lastRecord) throws SQLException{
		if (lastRecord.get(0).getId().contains("A")) {//回答一级问题
			...
			if(parameterin.size()>0) {//所回内容包括一级问涵括的三级参数
				...
				//回答是否删除
				//String answerclass=BaiduInstance.sentimentClassify(text);
				//if(answerclass.equals("")){
					
				//}else 
				if(text.contains("没")||text.contains("不")) {
					Set<Integer> set1=parametersdao.getparameteFromUpper(lastRecord.get(0).getId());
					Set<Integer> dealeduncheckparameter=lastRecord.get(0).getUncheckparameter();
					dealeduncheckparameter.removeAll(set1);
					lastRecord.get(0).setUncheckparameter(dealeduncheckparameter);
				}
				List<String> uncheckupperquestion=new LinkedList<String>();
				...
				if(maxtimesquestion.getCount()>1) {//还有一级问
					...
					infotag.setParameter(lastRecord.get(0).getParameter());
				}
				return infotag;
			}
		}else {//回答普通三级问
