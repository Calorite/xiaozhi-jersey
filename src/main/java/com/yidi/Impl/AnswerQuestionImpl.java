...
public class AnswerQuestionImpl implements AnswerQuestion {
	...
	@Override
	public ReturnInfo answerNormalQuestion(ReturnInfo lastinfo, AboutQuestionDAO questiondao,AboutSolutionDAO solutiondao, ConvertAdapter converter,
			Map<Integer, Parameter> allparameters, Map<Set<Integer>, ParameterSolution> parameter_solutionlist) {
		...
		if(relateParametemap.size()==1) {
			...
			ReturnInfo newinfotag=process.getReturnMSG(parameter_solutionlist, initalparameters,solutiondao, process);
			if (newinfotag!=null) {
			...
			ReturnInfo infotag2=process.getReturnMSG(parameter_solutionlist, vaildparameters, solutiondao, process);
